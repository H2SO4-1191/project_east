...
class StudentMyProfileScreen extends StatefulWidget {
  const StudentMyProfileScreen({super.key});

  @override
  State<StudentMyProfileScreen> createState() => _StudentMyProfileScreenState();
}

class _StudentMyProfileScreenState extends State<StudentMyProfileScreen> {
                  ...
                  child: ClipRRect(
                    ...
                    child: Image.network(
                      ...
                      loadingBuilder: (context, child, loadingProgress) {
                        ...
                        return const Center(
                          child: SmallLoadingIndicator(size: 24),
                        );
                      },
                    ),
                  ),
  ...
  Widget _buildAvatarPlaceholder(String text) {
    return Container(
      ...
      child: Center(
        child: Text(
          text.isNotEmpty ? text.substring(0, 1).toUpperCase() : 'S',
          style: const TextStyle(
            fontSize: 48,
            fontWeight: FontWeight.bold,
        ...
        actions: [
          if (_profileData != null && _profileData!['is_verified'] == true)
            IconButton(
              ...
              onPressed: () {
                Navigator.pushNamed(context, '/student/edit-profile/').then((_) => _loadProfile());
              },
            ...
            TextButton.icon(
              onPressed: () {
                Navigator.pushNamed(context, '/student/verify').then((_) => _loadProfile());
              },
                      ...
                      child: Column(
                        ...
                        children: [
                                    ...
                                    child: ClipOval(
                                      child: _getImageUrl(_profileData!['profile_image']).isNotEmpty
                                          ? Image.network(
                                              ...
                                              fit: BoxFit.cover,
                                              errorBuilder: (context, error, stackTrace) {
                                                final name = _profileData!['first_name'] ?? _profileData!['username'] ?? 'S';
                                                return _buildAvatarPlaceholder(name);
                                              },
                                            )
                                          : _buildAvatarPlaceholder(
                                              _profileData!['first_name'] ?? _profileData!['username'] ?? 'S',
                                            ),
                                    ),
                                  ),
                                ),
                          ...
                          _buildInfoCard(
                            ...
                            icon: Icons.school,
                            children: [
                              if (_profileData!['studying_level'] != null)
                                _buildInfoRow(
                                  icon: Icons.school_outlined,
                                  label: 'Studying Level',
                                  value: _profileData!['studying_level'].toString(),
                                ),
                            ],
                          ),
